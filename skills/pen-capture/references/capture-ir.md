...
`pen-capture-ir` version 1 is transported as a flat DOM snapshot. The converter rebuilds that flat transport into the original parent/child hierarchy using `path` and `parentPath`, with every child positioned relative to its reconstructed parent.
