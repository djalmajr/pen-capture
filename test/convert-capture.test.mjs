...
import { convertCaptureToPen, cssBackgroundToFill, cssColorToHex } from "../src/convert-capture.mjs";
...
describe("convertCaptureToPen", () => {
  test("preserves intentional clipping on the captured root", () => {
    const capture = {format:"pen-capture-ir",version:1,rootPath:"0",label:"Clipped shell",source:{},nodes:[
      {path:"0",parentPath:null,tag:"div",name:"root",text:null,rect:{x:0,y:0,width:320,height:200},attributes:{},styles:{...rootStyles,overflow:"hidden"}},
      {path:"0.0",parentPath:"0",tag:"div",name:"tall",text:null,rect:{x:0,y:0,width:100,height:400},attributes:{},styles:{...rootStyles,backgroundColor:"transparent"}}
    ]};
    expect(convertCaptureToPen(capture).root).toMatchObject({type:"frame",clip:true,width:320,height:200});
  });

  test("keeps transparent buttons bounded and maps unsupported Geist Variable text", () => {
    const capture = {format:"pen-capture-ir",version:1,rootPath:"0",label:"Ghost button",source:{},nodes:[
      {path:"0",parentPath:null,tag:"div",name:"root",text:null,rect:{x:0,y:0,width:120,height:64},attributes:{},styles:rootStyles},
      {path:"0.0",parentPath:"0",tag:"button",name:"button",text:"Ghost",textRect:{x:30,y:22,width:40,height:20},textRuns:[{text:"Ghost",rect:{x:30,y:22,width:40,height:20}}],rect:{x:20,y:16,width:80,height:32},attributes:{},styles:{...rootStyles,backgroundColor:"transparent",borderTopWidth:"0px",borderRightWidth:"0px",borderBottomWidth:"0px",borderLeftWidth:"0px",boxShadow:"none",color:"rgb(10,10,10)",fontFamily:"Geist Variable, sans-serif",fontSize:"14px",fontWeight:"500",lineHeight:"20px",textAlign:"center",textTransform:"none"}}
    ]};
    const button = convertCaptureToPen(capture).root.children[0];
    expect(button).toMatchObject({type:"frame",layout:"none",x:20,y:16,width:80,height:32});
    expect(button.children[0]).toMatchObject({type:"text",content:"Ghost",fontFamily:"Inter"});
  });

  test("creates editable absolute layers", () => {
    const capture = { format:"pen-capture-ir", version:1, rootPath:"0", label:"Example card", source:{}, nodes:[
    ...
    const result = convertCaptureToPen(capture);
    expect(result.root).toMatchObject({ type:"frame", name:"Captured · Example card", clip:false });
    expect(result.root.children[0]).toMatchObject({ type:"frame", name:"Div · title", layout:"none", x:16, y:16, width:288, height:24 });
  ...
  test("converts SVG primitives and visible form values", () => {
    ...
    const capture = { format:"pen-capture-ir", version:1, rootPath:"0", label:"Chart", source:{}, nodes:[
      { path:"0", parentPath:null, tag:"div", name:"card", text:null, rect:{x:0,y:0,width:320,height:200}, attributes:{}, styles:rootStyles },
  ...
  test("keeps nested SVG paths in the SVG viewBox instead of page coordinates", () => {
    ...
    const capture = { format:"pen-capture-ir", version:1, rootPath:"0", label:"Icon", source:{}, nodes:[
    ...
    const svg = convertCaptureToPen(capture).root.children[0];
  ...
  test("keeps zero-dimension SVG strokes and positions SVG text", () => {
    ...
    const capture = { format:"pen-capture-ir", version:1, rootPath:"0", label:"SVG labels", source:{}, nodes:[
      { path:"0", parentPath:null, tag:"div", name:"root", text:null, rect:{x:0,y:0,width:200,height:100}, attributes:{}, styles:rootStyles },
  ...
  test("positions SVG text against the SVG viewport when its g wrapper is expanded", () => {
    ...
    const capture = {format:"pen-capture-ir",version:1,rootPath:"0",label:"Donut",source:{},nodes:[
    ...
    const text = convertCaptureToPen(capture).root.children[0].children[0].children[0];
  ...
  test("scales SVG strokes and converts progress circle dash arrays", () => {
    ...
    const capture = { format:"pen-capture-ir", version:1, rootPath:"0", label:"Progress", source:{}, nodes:[
    ...
    const circle = convertCaptureToPen(capture).root.children[0].children[0];
  ...
  test("turns shadcn shadow rings into editable card strokes", () => {
    ...
    const capture = {format:"pen-capture-ir",version:1,rootPath:"0",label:"Card",source:{},nodes:[
      {path:"0",parentPath:null,tag:"div",name:"card",text:null,rect:{x:0,y:0,width:320,height:200},attributes:{},styles},
    ]};
    expect(convertCaptureToPen(capture).root).toMatchObject({stroke:"#0C0C091A",strokeWidth:1,strokeAlignment:"inner"});
  });
  ...
  test("uses placeholder pseudo color and native control padding", () => {
    ...
    const capture = {format:"pen-capture-ir",version:1,rootPath:"0",label:"Form",source:{},nodes:[
    ...
    const value = convertCaptureToPen(capture).root.children[0].children[0];
  ...
  test("preserves anchor underline and href", () => {
    ...
    const capture = {format:"pen-capture-ir",version:1,rootPath:"0",label:"Link",source:{},nodes:[
    ...
    const anchor = convertCaptureToPen(capture).root.children[0];
    expect(anchor.children[0]).toMatchObject({content:"email settings",underline:true,href:"https://example.com/settings",metadata:{type:"pen-capture-link",href:"https://example.com/settings"}});
    expect(anchor.children[1]).toMatchObject({type:"frame",name:"A · email settings · Underline",x:0,y:19,width:92,height:1,fill:"#0C0C09",metadata:{type:"pen-capture-link-underline",href:"https://example.com/settings"}});
  });

  test("drops zero-geometry control shadows that Pen would render with defaults", () => {
    ...
    const capture = {format:"pen-capture-ir",version:1,rootPath:"0",label:"Input group",source:{},nodes:[
    ...
    const input = convertCaptureToPen(capture).root.children[0];
  ...
  test("keeps visible SVG descendants behind zero-sized responsive wrappers", () => {
    ...
    const capture = { format:"pen-capture-ir", version:1, rootPath:"0", label:"Responsive chart", source:{}, nodes:[
  ...
  test("preserves DOM hierarchy with coordinates relative to each parent", () => {
    ...
    const capture = { format:"pen-capture-ir", version:1, rootPath:"0", label:"Nested", source:{}, nodes:[
    ...
    expect(section).toMatchObject({ type:"frame", layout:"none", x:40, y:30, width:300, height:200 });
    expect(paragraph).toMatchObject({ type:"frame", layout:"none", x:20, y:20, width:260, height:32 });
    expect(paragraph.children[0]).toMatchObject({ type:"text", x:4, y:4, width:83.2, height:20, textGrowth:"fixed-width-height" });
  });

  test("keeps transparent semantic wrappers bounded without clipping overflowing text", () => {
    const styles = { ...rootStyles, backgroundColor:"transparent", color:"rgb(0, 0, 0)", fontSize:"14px", lineHeight:"20px" };
    const capture = { format:"pen-capture-ir", version:1, rootPath:"0", label:"Overflow", source:{}, nodes:[
    ...
    const label = convertCaptureToPen(capture).root.children[0];
    expect(label).toMatchObject({type:"frame",layout:"none",x:10,y:10,width:60,height:14});
    expect(label).not.toHaveProperty("clip");
  ...
  test("drops one-pixel accessibility controls while keeping their visible replacement", () => {
    ...
    const capture = { format:"pen-capture-ir", version:1, rootPath:"0", label:"Hidden", source:{}, nodes:[
      { path:"0", parentPath:null, tag:"div", name:"root", text:null, rect:{x:0,y:0,width:100,height:100}, attributes:{}, styles:rootStyles },
  ...
  test("drops sr-only semantic text instead of rendering it over visible content", () => {
    ...
    const capture = { format:"pen-capture-ir", version:1, rootPath:"0", label:"Accessible field", source:{}, nodes:[
  ...
  test("keeps intrinsically-sized single-line text unwrapped", () => {
    ...
    const capture = { format:"pen-capture-ir", version:1, rootPath:"0", label:"Empty state", source:{}, nodes:[
      { path:"0", parentPath:null, tag:"div", name:"root", text:null, rect:{x:0,y:0,width:320,height:120}, attributes:{}, styles:rootStyles },
    ...
    const text = convertCaptureToPen(capture).root.children[0].children[0];
  ...
  test("preserves separate editable direct-text line runs", () => {
    const styles = { ...rootStyles, backgroundColor:"transparent", color:"rgb(0, 0, 0)", fontFamily:"Noto Sans", fontSize:"14px", fontWeight:"400", lineHeight:"20px", textAlign:"start", textTransform:"none" };
    const capture = { format:"pen-capture-ir", version:1, rootPath:"0", label:"Inline text", source:{}, nodes:[
      { path:"0", parentPath:null, tag:"div", name:"root", text:null, rect:{x:0,y:0,width:320,height:120}, attributes:{}, styles:rootStyles },
    ...
    const paragraph = convertCaptureToPen(capture).root.children[0];
  ...
  test("uses clipping frames for transparent overflow wrappers", () => {
    ...
    const capture = { format:"pen-capture-ir", version:1, rootPath:"0", label:"Clipped", source:{}, nodes:[
      { path:"0", parentPath:null, tag:"div", name:"root", text:null, rect:{x:0,y:0,width:200,height:100}, attributes:{}, styles:rootStyles },
      { path:"0.0", parentPath:"0", tag:"span", name:"badge", text:null, rect:{x:10,y:10,width:80,height:20}, attributes:{}, styles },
    ] };
    expect(convertCaptureToPen(capture).root.children[0]).toMatchObject({type:"frame",clip:true,x:10,y:10,width:80,height:20});
  });
  ...
  test("creates editable image fills for img and CSS background URLs", () => {
    ...
    const capture = { format:"pen-capture-ir", version:1, rootPath:"0", label:"Assets", source:{}, nodes:[
  ...
  test("uses Pen-safe HTTP assets and marks canvas for materialization in extension mode", () => {
    const styles = {...rootStyles,backgroundColor:"transparent",backgroundImage:"none",objectFit:"cover"};
    const capture = {format:"pen-capture-ir",version:1,rootPath:"0",label:"Portable",source:{url:"https://example.com/page"},nodes:[
    ...
    const root = convertCaptureToPen(capture,{allowEmbeddedAssets:false}).root;
    expect(root.children[0]).toMatchObject({type:"frame",clip:true,metadata:{type:"pen-capture-image-filter",filter:"brightness(0.6) grayscale(1)"}});
    ...
    expect(root.children[1]).toMatchObject({type:"frame",name:"Canvas · Materialization required",metadata:{type:"pen-capture-unmaterialized-canvas",reason:"embedded-assets-disabled"}});
    expect(JSON.stringify(root)).not.toContain("data:image/");
  });

  test("preserves a CSS color blend overlay as an editable Pen fill", () => {
    const capture = {format:"pen-capture-ir",version:1,rootPath:"0",label:"Filtered hero",source:{},nodes:[
      {path:"0",parentPath:null,tag:"div",name:"root",text:null,rect:{x:0,y:0,width:320,height:180},attributes:{},styles:rootStyles},
    ...
    expect(convertCaptureToPen(capture).root.children[0]).toMatchObject({
  ...
  test("preserves LAB text colors and an empty colored badge bullet", () => {
    ...
    const capture = {format:"pen-capture-ir",version:1,rootPath:"0",label:"Status",source:{},nodes:[
    ...
    const root = convertCaptureToPen(capture).root;
  ...
  test("materializes a transparent repeating gradient as uniform editable stripes", () => {
    const capture = {format:"pen-capture-ir",version:1,rootPath:"0",label:"Live",source:{},nodes:[
  ...
  test("expands repeating linear gradients into Pen gradient stops", () => {
  ...
  test("resolves SVG linear-gradient paint servers with combined fill opacity", () => {
    ...
    const capture = {format:"pen-capture-ir",version:1,rootPath:"0",label:"Stock",source:{},nodes:[
    ...
    const area = convertCaptureToPen(capture).root.children[0].children.find((child) => child.type === "path");
