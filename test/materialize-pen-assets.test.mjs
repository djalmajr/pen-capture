...
import { imageMagickArgsForCssFilter, materializePenAssets } from "../src/materialize-pen-assets.mjs";
...
test("materializes image fills and deduplicates identical assets", async () => {
  const outputDir = await mkdtemp(join(tmpdir(), "pen-assets-"));
  ...
  const result = await materializePenAssets(nodes, {outputDir,relativePrefix:"./assets/test"});
